...
language = [line[5:]
            for line in os.listdir('/train_data') if line.startswith('test')]
...
for lang in language:
    ...
    os.system(
        f'python3 tools/infer_rec.py -c configs/rec/PP-OCRv3/PP-OCRv3_rec.yml -o Global.pretrained_model=output/my_ocr_model/best_accuracy  Global.infer_img=/train_data/test_{lang}')
    ...
    with open(os.path.join('./', 'ground_truths_and_predicts.txt'), 'r', encoding='utf-8') as f:
        ...
        split_truth_predict = [line.strip('\n').replace(
            '\t', ' ').split('.jpg ') for line in f]
        ground_truths = [' '.join(line[0].replace('_', ' ').split(' ')[
                                  :-1]) for line in split_truth_predict]
...
# ----------------------------------------------------------------
    ...
    os.system(
        f'wer -c Ground_truths.txt Predicts.txt > result_{lang}_confusion_matrix.txt')
